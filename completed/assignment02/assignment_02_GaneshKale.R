...
person_df1 <- read.csv('data/tidynomicon/person.csv',stringsAsFactors = TRUE) # with this I received same output as str(person_df2), so I passed TRUE value
str(person_df1)                               # seems default it takes FALSE value for stringsAsFactors
...
column_names = c("ward_precint", "ballots_cast", "registered_voters", "voter_turnout")
voter_turnout_df2 <- read_excel('data/G04ResultsDetail2004-11-02.xls', sheet = "Voter Turnout", skip = 2, col_names = column_names)
...
tables <- lapply(X = table_names, FUN = dbReadTable, conn = db)
print(tables)
